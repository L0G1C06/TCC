# apps/portal_da_transparencia/models/despesas.py
"""
Despesas — Dados Consolidados

Fonte    : Portal da Transparência — CGU
S3       : portal_da_transparencia/parquet/modulo=despesas/
Histórico: 2013 → 2026 (múltiplos arquivos/mês)

Dados consolidados de despesa orçamentária e extraorçamentária com
informações detalhadas sobre pagamentos, empenhos e restos a pagar.

Chaves de correlação:
    Código Pagamento     → despesas-execucao.Código Pagamento
    Código Empenho       → despesas-execucao.Código Empenho
...
class Despesas(DuckDBModel):
    """
    Despesas — Dados Consolidados — camada Bronze.
    Espelho fiel do parquet no S3. Nenhuma transformação aplicada.

    Uso:
        Despesas.scan(ano="2022", mes="05").fetchdf()
    """
    ...
    CODIGO_EMPENHO = StringField(
        ...
        description = "Código do empenho. Chave: despesas-execucao",
    )
    ...
    # ── Natureza da Despesa ───────────────────────────────────────────
    CODIGO_NATUREZA_DESPESA_COMPLETA = StringField(
        source_name = "Código Natureza Despesa Completa",
        description = "Código completo da natureza da despesa",
    )
    SUBITEM = StringField(
        source_name = "Subitem",
        description = "Subitem da natureza da despesa",
    )

    # ── Valores ───────────────────────────────────────────────────────
    VALOR_PAGO = MoneyField(
        source_name = "Valor Pago (R$)",
        description = "Valor pago — VARCHAR '1.234,56'. Silver → Decimal(15,2)",
    )
    VALOR_RESTOS_PAGOS = MoneyField(
        source_name = "Valor Restos a Pagar Pagos (R$)",
        description = "Valor de restos a pagar pagos — VARCHAR '1.234,56'. Silver → Decimal(15,2)",
    )
    VALOR_RESTOS_PAGOS_CANCELADO = MoneyField(
        source_name = "Valor Restos a Pagar Cancelado (R$)",
        description = "Valor de restos a pagar cancelados — VARCHAR '1.234,56'. Silver → Decimal(15,2)",
    )
    VALOR_RESTOS_PAGOS_INSCritos = MoneyField(
        source_name = "Valor Restos a Pagar Inscritos (R$)",
        description = "Valor de restos a pagar inscritos — VARCHAR '1.234,56'. Silver → Decimal(15,2)",
    )
