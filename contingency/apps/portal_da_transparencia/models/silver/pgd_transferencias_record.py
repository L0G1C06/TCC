...
class PGDTransferenciasRecord(models.Model):
    """
    Amostra de PGD Transferências extraídas do S3.
    """
    ...
    # ── Extraorçamentário ─────────────────────────────────────────────
    extraorcamentario = models.CharField(max_length=10, blank=True, null=True)
    ...
    def __str__(self):
        return f"{self.nome_orgao} - {self.descricao} ({self.valor_total})"
