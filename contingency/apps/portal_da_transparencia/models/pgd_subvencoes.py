# apps/portal_da_transparencia/models/pgd_subvencoes.py
"""
PGD — Subvenções

Fonte    : Portal da Transparência — CGU
S3       : portal_da_transparencia/parquet/modulo=pgd-subvencoes/
Histórico: 2013 → 2026 (múltiplos arquivos/mês)

Informações sobre subvenções concedidas pela Administração Pública Federal
a entidades privadas para execução de atividades de interesse público.
...
class PGDSubvencoes(DuckDBModel):
    """
    PGD — Subvenções — camada Bronze.
    Espelho fiel do parquet no S3. Nenhuma transformação aplicada.

    Uso:
        PGDSubvencoes.scan(ano="2022", mes="05").fetchdf()
    """

    dataset_path = "portal_da_transparencia/parquet/"
    modulo       = "pgd-subvencoes"
